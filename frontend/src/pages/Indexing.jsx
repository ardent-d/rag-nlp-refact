...
const Indexing = () => {
  ...
  const [indexMode, setIndexMode] = useState('flat'); // 设置默认值为 milvus 的第一个模式
  ...
  const [selectedProvider, setSelectedProvider] = useState(''); // 初始为空
  ...
  const fetchProviders = async () => {
    try {
      ...
      if (providersData.providers && providersData.providers.length > 0) {
        ...
        // 如果当前selectedProvider不在返回的providers里，自动切换到第一个
      ...
      setStatus('Error fetching providers');
    }
  };
  ...
  useEffect(() => {
    // 同步 vectorDb 和 selectedProvider
    setVectorDb(selectedProvider);
    
    // 安全地获取当前向量库的可用模式
    const modes = dbConfigs[selectedProvider]?.modes || [];
    if (modes.length > 0) {
  ...
  useEffect(() => {
    ...
    const fetchCollections = async () => {
      try {
        ...
        setStatus('Error fetching collections');
      }
    };
    if (selectedProvider) {
      fetchCollections();
  ...
  const getAvailableModes = () => {
    return dbConfigs[vectorDb]?.modes || [];
  };

  // providers 加载后自动切换到第一个
  useEffect(() => {
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                >
                  ...
                  {collections && collections.length > 0 ? collections.map(coll => (
