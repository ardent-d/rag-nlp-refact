...
const ParseFile = () => {
  ...
  const [loadingTool, setLoadingTool] = useState('pymupdf');
  ...
  const handleProcess = async () => {
    if (!file || !fileType || !loadingTool || !parsingOption) {
    ...
    try {
      ...
      formData.append('loading_tool', loadingTool);
  ...
  const handleFileSelect = (e) => {
    ...
    if (file) {
      setFile(file);
      const baseName = file.name.replace(/\.(pdf|md)$/i, '');
      setDocName(baseName);
      ...
      if (extension === 'md') {
        ...
        setLoadingTool('markdown');
      } else if (extension === 'pdf') {
        setFileType('pdf');
        setLoadingTool('pymupdf');
      }
    }
  };
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-3 space-y-4">
          <div className="p-4 border rounded-lg bg-white shadow-sm">
            <div>
              <label className="block text-sm font-medium mb-1">Upload PDF or Markdown</label>
              <input
                type="file"
                accept=".pdf,.md"
            ...
            <div className="mt-4">
              ...
              <select
                value={loadingTool}
                onChange={(e) => setLoadingTool(e.target.value)}
                className="block w-full p-2 border rounded"
              >
                {fileType === 'pdf' ? (
                  <>
            ...
            <div className="mt-4">
              ...
              <select
                ...
                className="block w-full p-2 border rounded"
              >
                {fileType === 'pdf' && loadingTool === 'camelot' ? (
                  <>
