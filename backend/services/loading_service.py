...
class LoadingService:
    ...
    def load_pdf(self, file_path: str, method: str, strategy: str = None, chunking_strategy: str = None, chunking_options: dict = None, page_range=None, extract_images=False, extract_tables=False) -> str:
        """
        ...
        try:
            if method == "pymupdf":
                return self._load_with_pymupdf(file_path, page_range=page_range, extract_images=extract_images, extract_tables=extract_tables)
            elif method == "pypdf":
                return self._load_with_pypdf(file_path)
            elif method == "pdfplumber":
                return self._load_with_pdfplumber(file_path, page_range=page_range, extract_images=extract_images, extract_tables=extract_tables)
    ...
    def _load_with_pymupdf(self, file_path: str, page_range=None, extract_images=False, extract_tables=False) -> str:
        text_blocks = []
        try:
                ...
                for page_num, page in enumerate(doc, 1):
                    ...
                    if extract_images:
                        for img_index, img in enumerate(page.get_images(full=True)):
                            ...
                            text_blocks.append({
                                "text": f"[Image page {page_num} #{img_index+1} type {img_ext}]",
                                "page": page_num,
                                "metadata": {"image_base64": img_b64, "image_ext": img_ext, "type": "image"}
                            })
                    # PyMuPDF 不支持表格直接提取
    ...
    def _load_with_pdfplumber(self, file_path: str, page_range=None, extract_images=False, extract_tables=False) -> str:
        text_blocks = []
        try:
            with pdfplumber.open(file_path) as pdf:
                self.total_pages = len(pdf.pages)
                pages_to_read = self._parse_page_range(page_range, len(pdf.pages))
                for page_num, page in enumerate(pdf.pages, 1):
                    if page_num not in pages_to_read:
                        continue
                    page_text = page.extract_text()
                    ...
                    if extract_images and page.images:
                        for img_index, img in enumerate(page.images):
                            # pdfplumber图片为dict，无法直接导出图片内容，需用PIL等二次处理
                            text_blocks.append({
                                "text": f"[Image page {page_num} #{img_index+1}]",
                                "page": page_num,
                                "metadata": {"image_bbox": img.get('bbox'), "type": "image"}
                            })
                    # 提取表格
                    if extract_tables:
                        ...
                        for tbl_index, table in enumerate(tables):
                            ...
                            text_blocks.append({
                                "text": f"[Table page {page_num} #{tbl_index+1}]\n{md}",
                                "page": page_num,
                                "metadata": {"type": "table"}
                            })
            self.current_page_map = text_blocks
